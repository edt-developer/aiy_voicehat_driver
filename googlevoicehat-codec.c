...
static struct snd_soc_dai_driver voicehat_dai = {
	...
	.capture = {
		...
		/* ICS43432 mics produce 24-bit left-justified data.
		 * S24_LE = 24-bit in 32-bit container (upper 24 bits data,
		 * lower 8 bits padding). This ensures proper volume levels. */
